...
# ElasticNet regression with cross-validated alpha and l1_ratio
model = ElasticNetCV(cv=kf, random_state=42, max_iter=10000, l1_ratio=[.1, .5, .7, .9, .95, .99, 1])
...
print(f"ElasticNet Regression ({k}-fold cross-validation, standardized features):")
...
nonzero_coefs.reindex(nonzero_coefs.abs().sort_values(ascending=False).index).to_csv('data/interim/elasticnet_nonzero_coefficients.csv', header=['coefficient'])
