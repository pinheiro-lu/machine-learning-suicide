...
print(f"Linear Regression (OLS, {k}-fold cross-validation, standardized features):")
...
print("\nOLS coefficients (sorted by absolute value):")
print(coefs_named.reindex(coefs_named.abs().sort_values(ascending=False).index).head(20))

# Save coefficients to file
coefs_named.reindex(coefs_named.abs().sort_values(ascending=False).index).to_csv('data/interim/ols_coefficients.csv', header=['coefficient'])
